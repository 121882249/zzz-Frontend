...
新版代码不能改变尚未成功应用更新的旧更新器。新增约 2.6 MB 的 `TokenPro-<版本>-Windows-incremental.exe`：只包含增量类文件和界面资源，不包含 Java 运行时或完整程序。
旧版本自动更新失败时，可退出 TokenPro 窗口，打开此增量入口并选择原来的 TokenPro.exe。它验证原版本、原文件哈希和内嵌增量，必要时请求 UAC，然后原地替换；不迁移、不降级、不关闭 Codex/Claude，也不改账号配置。原有本机桥接通过其认证接口临时暂停，失败或结束后由原用户权限恢复；如需手动恢复，会明确提示。

...
Settings backups use an explicit file allowlist and private files. They include channel configuration and thread ID/provider/model associations, never conversation text or session databases. A failed configuration or migration restores the pre-switch settings and attempts to reopen the previous channel. If rollback fails, the backup location is reported instead of claiming success.
